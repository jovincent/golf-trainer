...
// protobufjs is CommonJS — normalise the namespace/default across bundlers.
const protobuf: typeof protobufNs = (protobufNs as unknown as { default?: typeof protobufNs }).default ?? protobufNs;

// LaunchMonitor.proto — Garmin R10 message schema (from the gsp-r10-adapter reference).
const PROTO = `
syntax = "proto3";
...
message StatusResponse { optional State state = 1; }
message WakeUpRequest {}
message WakeUpResponse {
  optional ResponseStatus status = 1;
  enum ResponseStatus { SUCCESS = 0; ALREADY_AWAKE = 1; UNKNOWN_ERROR = 2; }
}
message TiltRequest {}
message TiltResponse { optional Tilt tilt = 1; }
message StartTiltCalibrationRequest {}
message StartTiltCalibrationResponse {
  optional CalibrationStatus status = 1;
  enum CalibrationStatus { STARTED = 0; IN_PROGRESS = 1; ERROR = 2; }
}
message ResetTiltCalibrationRequest { optional bool should_reset = 1; }
message ResetTiltCalibrationResponse {
  optional Status status = 1;
  enum Status { UNKNOWN = 0; CAN_RESET = 1; ALREADY_RESET = 2; RESET_SUCCESSFUL = 3; CANNOT_RESET = 4; }
}
message ShotConfigRequest {
  optional float temperature = 1; optional float humidity = 2; optional float altitude = 3;
  optional float air_density = 4; optional float tee_range = 5;
}
message ShotConfigResponse { optional bool success = 1; }
...
message SubscribeRequest { repeated AlertMessage alerts = 1; }
...
message AlertMessage { optional AlertNotification.AlertType type = 1; optional uint32 interval = 2; }
message AlertNotification {
  optional AlertType type = 1;
  optional AlertDetails AlertNotification = 1001;
  enum AlertType { ACTIVITY_START = 0; ACTIVITY_STOP = 1; LAUNCH_MONITOR = 8; }
}
...
message State {
  ...
  enum StateType { STANDBY = 0; INTERFERENCE_TEST = 1; WAITING = 2; RECORDING = 3; PROCESSING = 4; ERROR = 5; }
}
message CalibrationStatus {
  optional StatusType status = 1;
  optional CalibrationResult result = 2;
  enum StatusType { UNKNOWN = 0; IN_BOUNDS = 1; RECALIBRATION_SUGGESTED = 2; RECALIBRATION_REQUIRED = 3; }
  enum CalibrationResult { SUCCESS = 0; ERROR = 1; UNIT_MOVING = 2; }
}
message Error {
  optional ErrorCode code = 1;
  optional Severity severity = 2;
  optional Tilt deviceTilt = 3;
  enum ErrorCode { UNKNOWN = 0; OVERHEATING = 1; RADAR_SATURATION = 2; PLATFORM_TILTED = 3; }
  enum Severity { WARNING = 0; SERIOUS = 1; FATAL = 2; }
}
message Tilt { optional float roll = 1; optional float pitch = 2; }
message Metrics {
  ...
  enum ShotType { PRACTICE = 0; NORMAL = 1; }
}
message BallMetrics {
  ...
  enum SpinCalculationType { RATIO = 0; BALL_FLIGHT = 1; OTHER = 2; MEASURED = 3; }
  enum GolfBallType { UNKNOWN = 0; CONVENTIONAL = 1; MARKED = 2; }
}
message ClubMetrics {
...
export const WrapperProto = root.lookupType("LaunchMonitor.Proto.WrapperProto");

export interface R10Metrics {
  shot_id?: number;
  ball_metrics?: {
    launch_angle?: number; launch_direction?: number; ball_speed?: number;
    spin_axis?: number; total_spin?: number;
  };
  club_metrics?: {
    club_head_speed?: number; club_angle_face?: number; club_angle_path?: number; attack_angle?: number;
  };
}

/** Encode a WrapperProto plain object to bytes. */
export function encodeWrapper(obj: object): Uint8Array {
  return WrapperProto.encode(WrapperProto.create(obj)).finish();
}

/** Decode bytes to a WrapperProto plain object (keepCase field names). */
export function decodeWrapper(bytes: Uint8Array): any {
  return WrapperProto.toObject(WrapperProto.decode(bytes), { defaults: false, enums: Number });
}
