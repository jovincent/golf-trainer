    ...
    <div className="min-h-screen grid place-items-center p-4 sm:p-8">
      {state === "loading" && (
        <div className="flex items-center gap-3 text-ink/50">
          <Loader2 className="w-5 h-5 animate-spin" /> Chargement du partage…
        </div>
      )}
      ...
      {state === "error" && (
        <div className="card p-8 text-center max-w-sm grid gap-3">
          ...
          <h1 className="font-display text-lg">Partage introuvable</h1>
          <p className="text-sm text-ink/50">Ce lien a peut-être expiré ou n'existe pas.</p>
          <a href="/" className="text-sm font-semibold text-fairway hover:underline">Découvrir FlightLab →</a>
        </div>
      )}
      ...
      {state === "ok" && share && (
        <div className="grid gap-5 justify-items-center">
          ...
          <div className="flex items-center gap-2 share-actions">
            ...
            >
              Essayer FlightLab <ArrowRight className="w-4 h-4" />
            </a>
          </div>
        </div>
      )}
